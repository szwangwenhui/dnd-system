...
function Preview() {
  ...
  // 创建交互区块渲染器实例
  const interactionRenderer = window.createInteractionRenderer({
    forms,
    fields,
    blocks,
    projectId,
  ...
  const renderBlock = (block) => {
    ...
    if (block.subType) {
      return interactionRenderer.renderChildBlock(block, blockStyle, contentStyle, isPopupBlock ? PopupCloseButton : null);
    }

    if (block.type === '交互') {
      return interactionRenderer.renderInteractionBlock(block, blockStyle, contentStyle, isPopupBlock ? PopupCloseButton : null, isPopupBlock);
    }
