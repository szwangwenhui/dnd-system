...
function BlockTemplateSelector({ 
  projectId, 
  onSelect,  // (选择结果) => void
  onCancel   // () => void
}) {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    try {
      const list = await window.dndDB.getBlockTemplatesByProjectId(projectId);
      setTemplates(list);
    } catch (error) {
        ...
        <div className="px-6 py-4">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600 mb-4">请选择区块创建方式：</p>
              <button
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">从空白区块开始设计</div>
                  </div>
                </div>
              </button>
              <button
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">从已有区块模板快速创建</div>
                  </div>
                </div>
              </button>
            </div>
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <div className="text-gray-500">暂无区块模板</div>
                  <div className="text-sm text-gray-400 mt-1">请先在区块列表中将区块设为模板</div>
                </div>
              ) : (
                <>
                  <p className="text-sm text-gray-600">选择要使用的区块模板：</p>
                  <select
                  ...
                  >
                    ...
                    {templates.map(t => (
                      <option key={t.id} value={t.id}>
                        [{blockTypeLabels[t.blockType] || t.blockType}] {t.name}
                      </option>
                    ))}
                  </select>
                  {selectedTemplateId && (
                      ...
                      {(() => {
                        ...
                        return t ? (
                          <>
                            ...
                            <div><strong>区块类型：</strong>{blockTypeLabels[t.blockType] || t.blockType}</div>
                            {t.description && <div><strong>描述：</strong>{t.description}</div>}
                            <div><strong>子区块数：</strong>{t.children?.length || 0} 个</div>
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                ...
                <div className="ml-3">
                  ...
                  <div className="text-sm text-gray-500">包含位置、尺寸、背景、边框、子区块等</div>
                </div>
              </label>
              <label className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                <input
...
window.BlockTemplateSelector = BlockTemplateSelector;

console.log('[DND2] BlockTemplateSelector.jsx 加载完成');
